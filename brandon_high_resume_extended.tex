...
\resumeSubHeadingListStart
    \resumeSubheading
      {Consilio}{Portland, OR (Remote)}
      {Staff Cloud Engineer}{May 2025 -- Present}
      \resumeItemListStart
	\resumeItem{Complete Kubernetes Cluster Automation}
          {Worked in tandem with Automation Architect to implement Enterprise tooling for completely automated Kubernetes cluster lifecycle via Cluster API on vSphere, orchestrated by in-house Go tooling running on Harness CI/CD.}
      \resumeItemListEnd

    \resumeSubheading
      {Ride with GPS}{Portland, OR (Hybrid)}
